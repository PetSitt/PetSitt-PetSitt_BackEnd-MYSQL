...
function chatSocketRouter(io) {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("send_message", async (data) => {
      try{
        ...
        const me = await User.findOne({ userEmail: socket.data?.userEmail });
  ...
  router.get("/:roomId", authMiddleware, async (req, res) => {
    try {
      ...
      const otherId = (room.userId !== user.id) ? room.userId : room.sitter_userId; 
  ...
  router.post("/test", async (req, res) => {
    ...
    console.log(io._nsps.get('/').adapter.rooms.get('62d8f4872dcb0968c04b64c2').size);
